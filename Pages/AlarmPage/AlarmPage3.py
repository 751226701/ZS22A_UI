...
class AlarmPage(Common):
    ...
    def assert_debounce_time(self, value):
        expect(self.page.locator("div").filter(has_text=re.compile(r"^去抖动 s \(0~10\)$")).get_by_role("textbox")).to_have_value(value)
    ...
    def assert_alarm_interval_time(self, value):
        expect(self.page.locator("div").filter(has_text=re.compile(r"^报警间隔时间")).get_by_placeholder("请选择")).to_have_value(value)
    ...
    def assert_record_time(self, value):
        expect(self. page.locator(self.__record_time[0]).filter(has_text=self.__record_time[1]).
