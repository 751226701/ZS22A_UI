...
from Pages.SystemPage.SystemPage9 import SystemPage
from Common.ReadYaml import ReadYaml
from Common.AllurePretty import PrettyAllure
from Config.Config import Config

yaml_data = ReadYaml(os.path.join(Config.test_datas_dir, "test_data_21.yaml")).read()
logindata = yaml_data[0]
Trace = Config.trace21
pageobject = None
DOWNLOAD_FLAG = False
...
def login(pageobject, url, user, passwd):
    ...
    pageobject.get_by_role("menuitem", name="存储设置").click()
...
def page():
    ...
    with sync_playwright() as play:
        ...
        pageobject = None
        if Trace:
            context.tracing.stop(path="trace21.zip")
        else:
            pass
        context.close()
        browser.close()
...
class TestAlarm:
    ...
    def test_case_01(self, page, CaseData: dict):
        ...
        page.click_clear_data()
        page.click_confirm()
        page.click_disk_full_stop()
        page.click_disk_full_cover()
        page.click_download_log()
        page.set_max_record_time("12")
        page.set_capture_interval("25")
        page.set_capture_number("25")
        page.click_ftp()
        page.click_switch()
        page.set_server_address("111.111.111.111")
        page.set_port("6666")
        page.set_username("test")
        page.set_password("123456")
        page.set_remote_directory("Test")
